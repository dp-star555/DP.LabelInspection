...
internal static partial class RegionEditor
{
    private sealed class EditableRegion
    {
        ...
        [
            ...
            Description(
                "范围0–1，默认0.01（1%）。分母是当前条/空隙或QR单模块的内部检查面积，不是整幅ROI。调小更敏感，调大更宽松；通常需同时达到最小异常面积。不是单字字形差异比例。"
            )
        ]
        ...
        [
            ...
            Description(
                "范围0–8，默认1。排除条/空隙或QR模块边界附近的原图像素，并按条宽/模块尺寸限制。0表示不主动排除边缘；调大可减少边界误报，但可能漏掉边缘缺陷。不是单字的归一化膨胀容差。"
            )
        ]
        ...
        [
            ...
            Description(
                "范围0–5，默认0.18（18%）。差异=(容差后缺墨像素+多墨像素)/归一化参考墨迹面积；超过此值产生超差记录，并非OCR置信度。调小更严格，调大更宽松；局部明显损伤可能被整字面积稀释，不能只看此比例认定质量。"
            )
        ]
        ...
        [
            Category("03 单字外观（仅文字）"),
            DisplayName("单字膨胀容差（归一化像素）"),
            Description(
                "范围0–8，默认2；0关闭膨胀容差豁免。单位是归一化字图像素，不是原图像素。2对应5×5膨胀核，边缘2像素内的差异可能被忽略，包括面积很大但很薄的缺墨带。调大更宽松、可能漏检；调小更敏感。设0不关闭裁边缩放及±2像素自动对齐。"
            )
        ]
