...
internal static partial class RegionEditor
{
    private sealed class EditableRegion
    {
        ...
        [
            ...
            Description(
                "范围0–8，默认1。排除条/空隙或QR模块边界附近的原图像素，并按条宽/模块尺寸限制。0表示不主动排除边缘；调大可减少边界误报，但可能漏掉边缘缺陷。不是单字的归一化边缘容差。"
            )
        ]
        ...
        [
            ...
            Description(
                "范围0–5，默认0.18（18%）。差异=(计入的缺墨像素+多墨像素)/归一化参考墨迹面积；只触及笔画边缘带的印刷波动不计入，良品通常为0或接近0，因此可用一批良品实测后在其最大值之上留余量设较小阈值以检出局部缺墨。超过此值产生超差记录，并非OCR置信度。调小更严格，调大更宽松；局部损伤仍按整字面积折算比例。"
            )
        ]
