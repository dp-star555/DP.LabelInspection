...
internal static partial class RegionEditor
{
    private sealed class EditableRegion
    {
        ...
        [
            ...
            Description(
                "范围0–8，默认1。条/空隙或QR模块交界处的边缘带宽度（原图像素，一维码含条的上下端）：只触及边缘带的毛刺、渗墨视为印刷波动；深入内部的缺墨/多墨按完整面积计入，不再被缩小。QR整体墨迹扩散时自动加宽对应方向，上限为模块半宽的40%。半宽不超过边缘带的细条/细空隙只检查横贯整条宽度的断裂。调大更宽松；不是单字的归一化边缘容差。"
            )
        ]
