...
| BarcodeEdgeTolerance | 1 | 条/空隙边缘带宽度（含上下端）；只触及边缘带的差异不计入，深入内部的按完整面积计入；半宽不超过边缘带的细条只检查横贯断裂 |
...
| 严重褪色 | 150处缺陷，等级F | 读不出 | 读不出 |

LOT ID条码最窄元素只有2–3像素，已接近成像分辨率：细条上的斑点在干净与损伤标签上都存在，局部结果只作参考，建议提高该区域成像分辨率。
