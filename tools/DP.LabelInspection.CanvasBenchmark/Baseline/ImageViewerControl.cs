...
public sealed class ImageViewerControl : Control
{
    ...
    private CanvasGeometryLayer? _geometry;

    /// <summary>显示复制后的中立区域/轮廓快照；null清除几何，不使用厂商运行时。</summary>
    public void SetGeometry(CanvasGeometry? geometry)
    {
        var next = geometry == null ? null : new CanvasGeometryLayer(geometry);
        var old = _geometry;
        _geometry = next;
        old?.Dispose();
        Invalidate();
    ...
    public void MatchViewFrom(ImageViewerControl source)
    {
        ...
        if (
            ...
            || _bitmap == null
            || source._bitmap == null
            || _bitmap.Size != source._bitmap.Size
        )
            return;
    ...
    {
        var next = image == null ? null : DrawingImageConverter.ToBitmap(image);
        var old = _bitmap;
        _bitmap = next;
        old?.Dispose();
        SetGeometry(null);
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        _geometry?.Draw(e.Graphics, view, view.Width / _bitmap.Width);
        foreach (var region in _regions)
            DrawBox(
                e.Graphics,
                view,
                region.Bounds,
                region.Kind == RegionKind.Ignore ? Color.Gray : Color.RoyalBlue,
                _findings.Any(f => f.Bounds.HasValue && f.Bounds.Value.Equals(region.Bounds))
                    ? ""
                    : region.Name
            );
        foreach (var character in _characters)
            DrawBox(e.Graphics, view, character.Bounds, Color.ForestGreen, character.Character);
        foreach (
            ...
            DrawBox(
                ...
                view,
                group.Key,
                group.Any(item => item.Finding.Verdict == InspectionVerdict.Ng)
                    ? Color.Crimson
                    : Color.DarkOrange,
                ShowFindingLabels
                    ? string.Join(
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
        ...
        if (e.Button != MouseButtons.Left || _panStart.HasValue || !Viewport().Contains(e.Location))
            return;
    ...
    protected override bool IsInputKey(Keys keyData) =>
        (keyData & Keys.KeyCode) == Keys.Home || base.IsInputKey(keyData);
